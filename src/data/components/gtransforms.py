...
class GroupScale:
    """Rescales the input PIL.Image to the given 'size'. 'size' will be the size of the smaller
    edge. For example, if height > width, then image will be.
...
class GroupTenCropToTensor:
    def __init__(self):
        self.worker = lambda crops: torch.stack(
            [torchvision.transforms.ToTensor()(crop) * 255 for crop in crops]
        )
...
class ToTensor:
    # Returns [0-255] rather than torchvision's [0-1]
    def __init__(self):
...
class LoopPad:
    ...
    def __call__(self, tensor):
        ...
        # tensor = np.concatenate([tensor] + pad, 0)
